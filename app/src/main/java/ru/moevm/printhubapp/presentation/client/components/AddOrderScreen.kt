...
import ru.moevm.printhubapp.presentation.client.viewmodels.GetPrinthubsViewModel
...
fun AddOrderScreen(
    ...
    val viewModel: GetPrinthubsViewModel = hiltViewModel()
    val state by viewModel.state.collectAsState()
    viewModel.getPrinthubs()
        ...
        Column(
            ...
            Box {
                LazyColumn(
                    ...
                    when (state) {
                        is GetPrinthubsState.Success -> {
                            val printhubs = (state as GetPrinthubsState.Success).printhubs
                            items(printhubs.size) { index ->
                                PrintHubCard(printhub = printhubs[index], onNavigateTo = onNavigateTo)
                            }
                        }
